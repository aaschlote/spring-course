...
public class AuthorizationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			List<GrantedAuthority> grantedAuthoritites = new ArrayList<GrantedAuthority>();
			roles.forEach(role -> {
				grantedAuthoritites.add(new SimpleGrantedAuthority(role));
			}
			);
			
			Authentication authentication = new UsernamePasswordAuthenticationToken(email, null, grantedAuthoritites);
